...
rbind(logit= fitB1$coefficients, probit= fitB2$coefficients, resetClass.probit= fitB2$coefficients/0.5513)

fitB3 = update(fitB1, formula = .~.+ I(log(time)) )
summary(rats$time[rats$status == 1])
summary(rats$time[rats$status== 0])
cbind(rats, model.matrix(fitB3))
...
Devs = c(fitB1$null.deviance, fitB1$deviance, fitB3$deviance, update(fitB3, .~.+ I(rx*log(time)))$dev, fitB4$deviance)
Devs
round(-diff(Devs))
